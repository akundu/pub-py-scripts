...
rm db_server.log.9102;  ulimit -n 65536;  python db_server.py --db-file $QUEST_DB_STRING --port 9102 --log-level WARNING --heartbeat-interval 180 --workers 6 --enable-access-log > db_server.log.9102 2>&1 
...
while true; do date ; python fetch_symbol_data.py I:DJX I:RUT NDX --latest --db-path $QUEST_DB_STRING --latest --timezone PST; sleep 10; done

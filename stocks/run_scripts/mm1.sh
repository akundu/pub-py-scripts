...
python  scripts/polygon_realtime_streamer.py --symbols I:VIX I:VIX1D I:SPX I:NDX --feed quotes --redis-url redis://lin1.kundu.dev:6379 --symbols-per-connection 20 --log-level DEBUG --batch-interval 1 --feed both --poll-only --poll-interval 5 --no-db-write

 python fetch_all_data.py --symbols I:NDX I:SPX SPX NDX  --max-concurrent 5 --executor-type process --stock-executor-type process --client-timeout 180 --use-market-hours --data-source polygon  --db-path $QUEST_DB_STRING --continuous --use-market-hours --interval-multiplier 0.5

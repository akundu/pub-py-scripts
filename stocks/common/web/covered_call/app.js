...
function renderTableFromAPI(prefix, apiData) {
    ...
    // Build column order from API metadata FIRST (before checking data length)
    const apiColumns = metadata.columns || [];
    const columnOrder = [];
    ...
    const hiddenColumns = new Set([
        'price_change_pct', 'price_change_pc', 'price_above_curr', 'price_above_current',
        'premium_diff', 'prem_diff', 'days_to_expiry', 'l_days_to_expiry', 'long_days_to_expiry',
        'long_contracts_available', 'option_ticker', 'l_option_ticker', 'buy_cost',
        'net_premium_after_spread', 'spread_slippage', 'net_daily_premium_after_spread',
        'spread_impact_pct', 'liquidity_score', 'assignment_risk', 'option_type', 'l_prem_tot'
    ]);

    // Define columns that should always be hidden (never shown)
    const alwaysHiddenColumns = new Set([
        'l_cnt_avl', 'long_contracts_available',
        'spread', 'l_spread'
    ]);

    // Define column groups (Short/Long pairs)
    const columnGroups = {
        'strike_price': { short: ['strike_price'], long: ['l_strike', 'long_strike_price'], display: 'Strike Price' },
        'option_premium': { short: ['opt_prem.', 'opt_prem', 'option_premium'], long: ['l_opt_prem', 'l_prem', 'long_option_premium'], display: 'Option Premium' },
        'expiration_date': { short: ['expiration_date'], long: ['l_expiration_date', 'long_expiration_date'], display: 'Expiration Date' },
        'bid_ask': { short: ['bid:ask', 'bid_ask'], long: ['l_bid:ask', 'l_bid_ask', 'long_bid_ask'], display: 'Bid:Ask' },
        'iv': { short: ['iv', 'implied_volatility'], long: ['liv', 'long_implied_volatility', 'l_iv'], display: 'IV' },
        'delta': { short: ['delta'], long: ['l_delta', 'long_delta'], display: 'Delta' },
        'theta': { short: ['theta'], long: ['l_theta', 'long_theta'], display: 'Theta' },
        'premium_total': { short: ['s_prem_tot', 'short_premium_total'], long: ['net_premium'], display: 'Premium Total' },
        'daily_premium': { short: ['s_day_prem', 'short_daily_premium'], long: ['net_daily_premium', 'net_daily_premi'], display: 'Daily Premium' }
    };

    // Build column to group mapping
    const columnToGroup = {};
    const groupOrder = ['strike_price', 'expiration_date', 'option_premium', 'bid_ask', 'iv', 'delta', 'theta', 'premium_total', 'daily_premium'];

    for (const groupName of groupOrder) {
        if (columnGroups[groupName]) {
            const group = columnGroups[groupName];
            group.short.forEach(col => columnToGroup[col] = { group: groupName, side: 'short' });
            group.long.forEach(col => columnToGroup[col] = { group: groupName, side: 'long' });
        }
    }

    // Get column names from API metadata or data, excluding spread columns
    let allColumnNames = [];
    if (apiColumns.length > 0) {
        allColumnNames = apiColumns
            .map(colMeta => typeof colMeta === 'string' ? colMeta : (colMeta.name || colMeta))
            .filter(colName => colName !== 'spread' && colName !== 'l_spread');
    } else if (data.length > 0) {
        allColumnNames = Object.keys(data[0]).filter(colName => colName !== 'spread' && colName !== 'l_spread');
    }

    // Build group header row (first row)
    const groupHeaderRow = document.createElement('tr');
    groupHeaderRow.className = 'group-header-row';

    // Build column header row (second row)
    ...
    function createColumnHeader(colName, idx, prefix, isHidden, isAlwaysHidden, shortLongLabel, groupName) {
        ...
        if (isAlwaysHidden) {
            th.classList.add('always-hidden');
        } else if (isHidden) {
            th.classList.add('is-hidden-col');
        }

        if (shortLongLabel) {
            th.classList.add('grouped-column');
            const displaySpan = document.createElement('span');
            displaySpan.className = 'column-name-display';
            displaySpan.textContent = colName.substring(0, 15);
            th.appendChild(displaySpan);

            const shortLongSpan = document.createElement('span');
            shortLongSpan.className = 'column-name-short-long';
            shortLongSpan.textContent = shortLongLabel;
            th.appendChild(shortLongSpan);

            const filterableSpan = document.createElement('span');
            filterableSpan.className = 'column-name-filterable';
            filterableSpan.textContent = colName;
            th.appendChild(filterableSpan);
        } else {
            th.textContent = colName.substring(0, 15);
        }

        return th;
    }

    // Process columns in order, creating group headers where needed
    for (let idx = 0; idx < allColumnNames.length; idx++) {
        const colName = allColumnNames[idx];
        if (processedCols.has(colName)) continue;

        const normalizedColName = colName.toLowerCase().replace(/[_\s-]/g, '_');
        const isAlwaysHidden = alwaysHiddenColumns.has(colName) || alwaysHiddenColumns.has(normalizedColName);
        const isHidden = !isAlwaysHidden && (hiddenColumns.has(colName) || hiddenColumns.has(normalizedColName));

        const colInfo = columnToGroup[colName];

        if (colInfo && colInfo.side === 'short') {
            // This is a grouped column - find the long column
            const group = columnGroups[colInfo.group];
            const longCol = allColumnNames.find(c => group.long.includes(c) && !processedCols.has(c));
            ...
            if (longCol) {
                ...
                groupHeader.textContent = group.display;
                if (isHidden || isAlwaysHidden) {
                    groupHeader.classList.add(isAlwaysHidden ? 'always-hidden' : 'is-hidden-col');
                }
                groupHeaderRow.appendChild(groupHeader);

                // Create short column header
                const shortTh = createColumnHeader(colName, idx, prefix, isHidden, isAlwaysHidden, 'Short', colInfo.group);
                columnHeaderRow.appendChild(shortTh);
                processedCols.add(colName);
                columnOrder.push(colName);
                columnMetadata.push({ name: colName, isHidden, isAlwaysHidden });

                // Create long column header
                const longNormalized = longCol.toLowerCase().replace(/[_\s-]/g, '_');
                const longIsAlwaysHidden = alwaysHiddenColumns.has(longCol) || alwaysHiddenColumns.has(longNormalized);
                const longIsHidden = !longIsAlwaysHidden && (hiddenColumns.has(longCol) || hiddenColumns.has(longNormalized));
                const longIdx = allColumnNames.indexOf(longCol);
                const longTh = createColumnHeader(longCol, longIdx, prefix, longIsHidden, longIsAlwaysHidden, 'Long', colInfo.group);
                columnHeaderRow.appendChild(longTh);
                processedCols.add(longCol);
                columnOrder.push(longCol);
                columnMetadata.push({ name: longCol, isHidden: longIsHidden, isAlwaysHidden: longIsAlwaysHidden });
            } else {
                // Long column not found, treat as ungrouped
                const emptyGroupHeader = document.createElement('th');
                emptyGroupHeader.className = 'group-header';
                emptyGroupHeader.colSpan = 1;
                emptyGroupHeader.style.background = 'transparent';
                emptyGroupHeader.style.border = 'none';
                if (isHidden || isAlwaysHidden) {
                    emptyGroupHeader.classList.add(isAlwaysHidden ? 'always-hidden' : 'is-hidden-col');
                }
                groupHeaderRow.appendChild(emptyGroupHeader);

                const th = createColumnHeader(colName, idx, prefix, isHidden, isAlwaysHidden, null, null);
                columnHeaderRow.appendChild(th);
                processedCols.add(colName);
                columnOrder.push(colName);
                columnMetadata.push({ name: colName, isHidden, isAlwaysHidden });
            }
        } else if (!colInfo || colInfo.side !== 'long') {
            // Ungrouped column
            const emptyGroupHeader = document.createElement('th');
            emptyGroupHeader.className = 'group-header';
            emptyGroupHeader.colSpan = 1;
    ...
    // If we still don't have columnOrder, something went wrong
    if (columnOrder.length === 0) {
        console.error('No columns found in API metadata or data');
        tbody.innerHTML = '<tr><td colspan="100%" style="text-align: center; padding: 20px; color: red;">Error: No column information available.</td></tr>';
        return;
    }
    ...
    function formatCellValue(value, colName) {
        ...
        if (typeof value === 'string' && (normalizedCol.includes('expiration') || normalizedCol.includes('exp_date') || normalizedCol.includes('date'))) {
            ...
            // Handle formats like "2025-12-12 00:00:00" or "2025-12-12T00:00:00" or "2025-12-12T00:00:00+00:00"
            const dateMatch = value.match(/^(\d{4}-\d{2}-\d{2})/);
            if (dateMatch) {
                return {
                    display: dateMatch[1], // Just the date part (YYYY-MM-DD)
                    raw: dateMatch[1] // Store only date in raw too
                };
            }
...
function evaluateFilter(filter, row, tableId) {
    ...
    // Check if this is an expiration date column
    const isExpDateCol = isExpirationDateColumn(filter.field);

    // For expiration date columns, always extract date portion and compare dates
    if (isExpDateCol) {
        const cellDate = getDateValue(cell);

        // Extract date portion from filter value (handles various formats)
        const filterValueStr = String(filter.value).trim();
        const filterDate = extractDatePortion(filterValueStr);

        // If filter value is not a valid date, can't compare
        if (!filterDate) {
            // For != operator, if filter is not a date, consider it a mismatch
            if (filter.operator === '!=') return cellDate !== null;
            return false; // Other operators require valid dates
        }

        // If cell has no date
        if (cellDate === null) {
            // Only == and != operators make sense for null dates
            if (filter.operator === '==') return false;
            if (filter.operator === '!=') return true;
            return false; // Other operators don't make sense for null dates
        }

#!/usr/bin/env python3
"""
Equity Price Movement Analyzer

Analyzes price movements from a given time of day to market close,
or close-to-close if no time specified. Outputs histogram visualization
and statistical summary with percentiles.
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Close-to-close analysis for SPY over entire dataset
  python scripts/analyze_price_movements.py --ticker SPY

  # Movement from 10:00 AM PST to close for specific date range
  python scripts/analyze_price_movements.py --ticker QQQ \\
      --from-time 10:00 --timezone PST \\
      --start-date 2025-10-01 --end-date 2026-01-29

  # NDX index, EST timezone, no plot
  python scripts/analyze_price_movements.py --ticker I:NDX \\
      --from-time 11:30 --timezone EST --no-plot
        """
    )
...
def get_daily_data(df: pd.DataFrame) -> dict:
    ...
    """
    # Add date column based on when market closes (ET)
    et_tz = ZoneInfo('America/New_York')
...
def main():
    ...
    # Load data
    print(f"Loading data for {args.ticker} from {args.data_dir}/...")
    try:
        df = load_ticker_data(args.data_dir, args.ticker, args.start_date, args.end_date)
    ...
    # Calculate movements
    if args.from_time:
        print(f"Analyzing movements from {args.from_time} {args.timezone} to close...")
    else:
        print("Analyzing close-to-close movements...")

    movements = calculate_movements(df, args.from_time, args.timezone, args.day_direction)
    ...
    print_statistics(stats, args.ticker, start_date_str, end_date_str,
                    args.from_time, args.timezone, args.day_direction)

    # Generate histogram if not disabled
    if not args.no_plot:
        generate_histogram(movements, stats, args.ticker, start_date_str, end_date_str,
                          args.from_time, args.timezone, args.output)
